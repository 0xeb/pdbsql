    ...
    std::string& output)
{
    ...
    if (input == ".help") {
        output = "PDBSQL Commands:\n"
                 ...
                 "  Show functions that start with 'Init'\n"
...
#else
        output = "AI agent support not compiled in. Rebuild with -DPDBSQL_WITH_AI_AGENT=ON\n";
#endif
        return CommandResult::HANDLED;
    }
...
#else
    return {false, "Error: AI agent not compiled in. Rebuild with -DPDBSQL_WITH_AI_AGENT=ON\n", 1};
#endif
}

...
/**
 * AIAgent - Natural language interface for PDBSQL using libagents
 *
 * This is a simplified wrapper around libagents that provides:
 * - Tool registration for SQL execution
 * - Main-thread tool dispatch via query_hosted() (required for thread safety)
...
private:
    ...
    /// Register the pdbsql tool with libagents
    void setup_tools();

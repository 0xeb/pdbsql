...
class PdbsqlHTTPServer::Impl {
public:
    httplib::Server svr;
    std::thread server_thread;
};

PdbsqlHTTPServer::PdbsqlHTTPServer() = default;

PdbsqlHTTPServer::~PdbsqlHTTPServer() {
    stop();
}

std::string PdbsqlHTTPServer::queue_and_wait(const std::string& sql) {
    if (!running_.load()) {
...
int PdbsqlHTTPServer::start(int port, HTTPQueryCallback query_cb,
    ...
    svr.Get("/", [port](const httplib::Request&, httplib::Response& res) {
        std::string welcome = "PDBSQL HTTP Server (REPL)\n\nEndpoints:\n"
...
void PdbsqlHTTPServer::complete_pending_commands(const std::string& result) {
...
std::string PdbsqlHTTPServer::url() const {

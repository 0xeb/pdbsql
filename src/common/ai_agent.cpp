...
// Embedded documentation from prompts/pdbsql_agent.md
...
void AIAgent::setup_tools() {
    libagents::Tool pdbsql_tool;
    pdbsql_tool.name = "pdbsql";
    pdbsql_tool.description =
        "Execute a SQL query against a PDB (Program Database) file. "
        "Available tables: functions, publics, data, udts, enums, typedefs, thunks, labels, "
        "compilands, source_files, line_numbers, sections, udt_members, enum_values, "
        "base_classes, locals, parameters. "
        "Example: SELECT name, rva, length FROM functions WHERE name LIKE '%main%' ORDER BY length DESC LIMIT 10";

    pdbsql_tool.parameters_schema = R"({
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "SQL query to execute against the PDB database"
            }
        },
        "required": ["query"]
    })";
    ...
    pdbsql_tool.handler = [this](const std::string& args) -> std::string {
        try {
    ...
    agent_->register_tool(pdbsql_tool);

    if (verbose_) {
        std::cerr << "[AGENT] Registered pdbsql tool" << std::endl;
    }
}
...
std::string AIAgent::build_primed_message(const std::string& user_message) {
    return std::string(SYSTEM_PROMPT) +
        "\n\n---\n\n"
        "# User Request\n\n"
        "Use the `pdbsql` tool to execute SQL queries. "
        "Do not use Bash, Grep, or other tools - only use the pdbsql tool.\n\n" +
        user_message;
}

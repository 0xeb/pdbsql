...
// Agent settings stored in ~/.pdbsql/agent_settings.json
...
// Get the settings directory path (~/.pdbsql or %APPDATA%\pdbsql)
inline std::string GetSettingsDir() {
#ifdef _WIN32
    char path[MAX_PATH];
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_APPDATA, nullptr, 0, path))) {
        return std::string(path) + "\\pdbsql";
    }
    // Fallback to USERPROFILE
    const char* userprofile = std::getenv("USERPROFILE");
    if (userprofile) {
        return std::string(userprofile) + "\\.pdbsql";
    }
    return ".pdbsql";
#else

...
/**
 * PdbsqlMCPServer - MCP server for PDBSQL
 *
...
class PdbsqlMCPServer {
public:
    PdbsqlMCPServer();
    ~PdbsqlMCPServer();

    // Non-copyable
    PdbsqlMCPServer(const PdbsqlMCPServer&) = delete;
    PdbsqlMCPServer& operator=(const PdbsqlMCPServer&) = delete;
